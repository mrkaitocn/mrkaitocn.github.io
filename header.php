 ...
 * Version:            1.0.0
 * Theme Name:         vapier
 * Theme URI:          http://wpbingosite.com/themes/vapier/
 * Author:             Wpbingo
...
<?php 
	...
	$direction = vapier_get_direction(); 
	$vapier_page_id = get_the_ID();	
	...
	$enable_sticky_header = ( isset($vapier_settings['enable-sticky-header']) && $vapier_settings['enable-sticky-header'] ) ? ($vapier_settings['enable-sticky-header']) : false;
	$show_minicart = (isset($vapier_settings['show-minicart']) && $vapier_settings['show-minicart']) ? ($vapier_settings['show-minicart']) : false;
	$show_searchform = (isset($vapier_settings['show-searchform']) && $vapier_settings['show-searchform']) ? ($vapier_settings['show-searchform']) : false;
	$background_page = get_post_meta( get_the_ID(), 'page_background', true );
	$checkout_page_style="";
	if( function_exists('is_checkout') && is_checkout() ){
		$checkout_page_style = vapier_get_config('checkout_page_style','checkout-page-style-1');
	}
?>
<!--<![endif]-->
<head>
							...
							<?php if($show_minicart || $show_searchform){ ?>
								...
								<div class="col-xl-2 col-lg-2 col-md-6 col-sm-6 col-6 margin-top-20">
									...
									<div class="search-box pull-right">
										...
										<div class="dropdown-search"><?php vapier_search_form_product(); ?></div>
									</div>
									<?php } ?>
									<!-- End Search -->
								</div>
