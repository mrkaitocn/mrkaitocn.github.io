...
function vapier_custom_css() {
	global $vapier_page_id;
	$vapier_settings = vapier_global_settings();
	if (!is_admin()) {
		wp_enqueue_style( 'vapier-style-template', get_template_directory_uri().'/css/template.css');
		ob_start(); 
		...
		wp_add_inline_style( 'vapier-style-template', implode($custom_css) );
	}
}
add_action('wp_enqueue_scripts', 'vapier_custom_css' );
function vapier_custom_js() {
	if (!is_admin()) {
		wp_enqueue_script( 'vapier-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery'), null, true );
		wp_localize_script( 'vapier-script', 'vapier_ajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
		...
		wp_add_inline_script( 'vapier-script', $custom_js);
	}
}
add_action('wp_enqueue_scripts', 'vapier_custom_js' );